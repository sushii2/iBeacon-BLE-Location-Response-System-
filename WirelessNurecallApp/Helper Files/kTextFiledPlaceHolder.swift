...
//  WirelessNurecallApp
//
//  Created by Saksham Saraswat on 5/24/19.
...
class kTextFiledPlaceHolder: UITextField {
    ...
    func placeholderText(_ placeholder: NSString){
        let atts: [String: Any]  = [NSAttributedString.Key.foregroundColor.rawValue: UIColor.lightGray, NSAttributedString.Key.font.rawValue: UIFont.labelFontSize]
        self.attributedPlaceholder = String(string: placeholder as String , attributes:atts)
        self.EnableMaterialPlaceHolder(enableMaterialPlaceHolder: self.enableMaterialPlaceHolder)
    }
